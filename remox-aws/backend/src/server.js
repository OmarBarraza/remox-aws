...
app.get("/api/propiedades", async (req, res) => {
  try {
    ...
    return res.status(500).json({
      ok: false, error: "No se pudo consultar FetchMapData", detail: error.message,
    });
  }
});
...
app.get("/api/propiedad/:id", async (req, res) => {
  try {
    ...
    return res.status(500).json({
      ok: false, error: "No se pudo consultar FetchPropiedadFlyerData", detail: error.message,
    });
  }
});
...
app.listen(PORT, () => {
  console.log(`✅  Remox proxy corriendo en http://localhost:${PORT}`);
});
